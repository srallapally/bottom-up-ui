...
onMounted(() => {
  ...
  // Start the mining process (do not await so timer updates during the blocking call)
  startMining();
});
...
const startMining = async () => {
  ...
  try {
    // Call mining via results store (POST /api/sessions/{id}/mine)
    await resultsStore.mine(
...
const checkStatus = async () => {
  try {
    ...
    const results = await resultsStore.loadResults(sessionStore.currentSession.session_id);
