<!-- client/src/views/browse/EntitlementsView.vue -->
<template>
...
onMounted(async () => {
  try {
    const data = await api.browseData(sessionStore.sessionId, 'entitlements');
    ...
    error.value = e.userMessage || 'Failed to load entitlements';
  } finally {
    loading.value = false;
  }
});
